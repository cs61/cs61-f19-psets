...
$info = run_sh61("./simpong61 -d0.01 -w13 -h8 -b10 -s20", "stdin" => "/dev/null", "stdout" => "pipe", "time_limit" => 1, "size_limit" => 10000);
if (!exists($info->{"killed"}) || $info->{"killed"} !~ /^timeout/) {
    print OUT "${Red}FAILURE${Redctx} (expected timeout, got ",
        (exists($info->{"killed"}) ? $info->{"killed"} : "exit status " . $info->{"status"}),
        ")${Off}\n";
}
my($tt) = sprintf("(%.3fs real, %.3fs/%.0f%% user)", $info->{"time"}, $info->{"utime"}, 100 * $info->{"utime"} / $info->{"time"});
